...
- [[#if-you-cannot-install-magit-or-downloading-packages-complains-about-missing-public-key][If You Cannot Install Magit or Downloading Packages Complains about Missing Public Key]]
  - [[#you-can-try-to-get-the-public-key-manually][You can Try to Get the Public Key Manually]]
  - [[#install-packages-without-signature][Install packages without signature]]
- [[#config-example-references][Config Example References]]
...
* If You Cannot Install Magit or Downloading Packages Complains about Missing Public Key
  
** You can Try to Get the Public Key Manually
...
** Install packages without signature
...
Then manually do M-x package-install and install 'gnu-elpa-keyring-update'

* Config Example References
...
#+begin_src emacs-lisp
(use-package magit
    :after (general magit-section with-editor)
    :ensure t
    :config
        (th/leader-keys ; Magit Helpers
          "g"   '(:ignore t :wk "Magit")
          "g g" '(magit-status :wk "Get into Magit buffer for Current Project")
          ))
#+end_src
